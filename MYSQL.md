...
+ [Типы данных в MySQL](#dataTypes)
+ [Свойства столбцов](#columnProperties)
...
для установки кодировки по умолчанию, отличную от кодировки, указанной при настройке MySQL, то при создании базы данных вы можете указать нужную кодировку (character set) и/или правило сравнения (сортировки) символьных значений:
...
+ просмотреть список используемых в MySQL кодировок-- `SHOW CHARACTER SET;`

+ увидеть список правил сравнения символьных значений-- `SHOW COLLATION;`
...
+ удалить ненужную или ошибочно созданную базу данных-- `DROP DATABASE <Имя базы данных>;`

+ выбрать одну из баз, созданных на данном сервере MySQL, в качестве текущей базы данных-- `USE <Имя базы данных>;`

+ увидеть список всех баз, существующих на данном сервере MySQL-- `SHOW DATABASES;`
...
<a name="dataTypes"><h2>Типы данных в MySQL</h2></a>
...
<a name="columnProperties"><h2>Свойства столбцов</h2></a>
...
+ COMMENT 'Текст комментария'
	...
	Произвольное текстовое описание столбца длиной до 255 символов. Например, описание для поля rating таблицы Customers можно задать следующим образом: `rating INT COMMENT 'Рейтинг клиента';`
...
`ALTER TABLE <Имя таблицы> DROP <Имя столбца>;`
	...
	При удалении столбца он удаляется также из всех индексов, в которые он был включен (в отличие от первичного и внешнего ключа, которые требуется удалить прежде, чем удалять
входящие в них столбцы). Если при этом в индексе не остается столбцов, то индекс также автоматически удаляется.
